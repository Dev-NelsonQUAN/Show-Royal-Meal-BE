...
export const signUp = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // 2. Perform a type check
    if (error instanceof Error) {
      errorMessage = error.message;
    }
    // 3. Use the safe message
...
export const login = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!isMatch) {
      res
        .status(401)
        .json({ message: "Incorrect password. Invalid credentials." });
      return;
    }
...
// // This signUp function is already correct.
// export const signUp = async (req: Request, res: Response): Promise<void> => {
//   try {
//     const { fullName, email, password, phoneNumber, role = "User" } = req.body;
//     if (!fullName || !email || !password || !phoneNumber) {
...
//     res.status(201).json({ message: "User created successfully", user });
//   } catch (error: unknown) { // Typed as any to safely access message
//     console.error("Signup error:", error);
//     // FIX: Send the error MESSAGE, not the whole object.
...
//     const isMatch = await bcrypt.compare(password, user.password);
//     if (!isMatch) {
//       res.status(401).json({ message: "Incorrect password. Invalid credentials." });
//       return;
//     }
...
//     res.status(200).json({ message: "Login Successful", user, token });
//   } catch (error: any) { // Typed as any to safely access message
//     console.error("Login error:", error);
//     // FIX: Send the error MESSAGE, not the whole object.
//     res.status(500).json({ message: error.message || "An internal server error occurred." });
//   }
